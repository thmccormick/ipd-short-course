...
subtitle: "Prediction-Based Inference: Methods & Applications"
description:  |
  Build intuition for prediction-based inference by simulating data and 
...
- Introduce the [`ipd`](https://github.com/ipd-tools/ipd) package,
...
Throughout the workshop, we will use reproducible seeds and [`tidyverse`](https://www.tidyverse.org/) conventions.
...
* `chen`: Chen and Chen Estimator ([Gronsbell et al., 2026](https://arxiv.org/pdf/2411.19908))
* `pdc`: Prediction Decorrelated Inference ([Gan et al., 2024](https://onlinelibrary.wiley.com/doi/10.1111/anzs.12429))
...
* `ppi_a`: PPI using All Data ([Gronsbell et al., 2026](https://arxiv.org/pdf/2411.19908))
* `pspa`: Assumption-Lean and Data-Adaptive Post-Prediction Inference ([Miao et al., 2024](https://www.jmlr.org/papers/volume26/24-0056/24-0056.pdf))
...
The `ipd` package provides a function for simulating data based on the approach in [Wang et al. (2020)](https://www.pnas.org/doi/10.1073/pnas.2001238117). We provide more detail about this function at the end of this model. Here, we will be generating synthetic data following [Gronsbell et al. (2026)](https://arxiv.org/pdf/2411.19908) and [Ji et al. (2025)](https://arxiv.org/pdf/2501.09731) to better illustrate the core principles of PB inference in a more straightforward manner. We provide helper functions below to simulate data from a linear regression model.
...
* A `sigma_y = 1` outcome error variance
...
The single wrapper function `ipd()` implements multiple PB inference methods (e.g., **Chen & Chen**, **PostPI**, **PPI**, **PPI++**, **PSPA**) for various
inferential tasks (e.g., **mean** and **quantile** estimation, **ols**, **logistic**, and **Poisson** regression).  
...
Let's run one method, `chen`, proposed by [Gronsbell et al. (2026)](https://arxiv.org/pdf/2411.19908). The **Chen + Chen** estimator is a PB method that combines information from:
...
* An **unlabeled set** (where $Y$ is *presumed missing* but you compute 
  predictions $f$).
...
`simdat` function, but this optional activity gives you a hands-on implementation.
